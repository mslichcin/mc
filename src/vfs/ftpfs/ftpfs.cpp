                    ...
                    const vfs_path_t * vpath, const vfs_path_element_t * vpath_element)
{
    ...
    super->root =
        vfs_s_new_inode (vpath_element->vclass, super, ftpfs_default_stat (vpath_element->vclass));

    return ftpfs_open_archive_int (vpath_element->vclass, super);
}
                    ...
                    struct sockaddr_storage data_addr, socklen_t data_addrlen)
{
    ...
    addr = (char*)g_try_malloc (NI_MAXHOST);
    if (addr == NULL)
        ERRNOR (ENOMEM, -1);
...
ftpfs_send_command (const vfs_path_t * vpath, const char *cmd, int flags)
{
    ...
    p = ftpfs_translate_path (path_element->vclass, super, rpath);
    r = ftpfs_command (path_element->vclass, super, WAIT_REPLY, cmd, p);
    g_free (p);
    ...
    if (r != COMPLETE)
    {
        path_element->vclass->verrno = EPERM;
        return -1;
    }
    if (flush_directory_cache)
        vfs_s_invalidate (path_element->vclass, super);
    return 0;
}
...
init_ftpfs (void)
{
    ...
    ftpfs_subclass.flags = (vfs_subclass_flags_t)(VFS_S_REMOTE | VFS_S_USETMP);
