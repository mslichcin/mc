                   ...
                   const vfs_path_t * vpath, const vfs_path_element_t * vpath_element)
{
    ...
    return fish_open_archive_int (vpath_element->vclass, super);
}
...
fish_rename (const vfs_path_t * vpath1, const vfs_path_t * vpath2)
{
    ...
    ret =
        fish_send_command (path_element->vclass, super2, OPT_FLUSH, SUP->scr_mv,
...
fish_link (const vfs_path_t * vpath1, const vfs_path_t * vpath2)
{
    ...
    ret =
        fish_send_command (path_element->vclass, super2, OPT_FLUSH, SUP->scr_hardlink,
                           "FISH_FILEFROM=%s FISH_FILETO=%s;\n", rpath1, rpath2);
...
fish_symlink (const vfs_path_t * vpath1, const vfs_path_t * vpath2)
{
    ...
    ret =
        fish_send_command (path_element->vclass, super, OPT_FLUSH, SUP->scr_ln,
                           "FISH_FILEFROM=%s FISH_FILETO=%s;\n", qsetto, rpath);
...
fish_chmod (const vfs_path_t * vpath, mode_t mode)
{
    ...
    ret =
        fish_send_command (path_element->vclass, super, OPT_FLUSH, SUP->scr_chmod,
...
fish_chown (const vfs_path_t * vpath, uid_t owner, gid_t group)
{
    ...
    ret =
        fish_send_command (path_element->vclass, super, OPT_FLUSH, SUP->scr_chown,
...
fish_utime (const vfs_path_t * vpath, mc_timesbuf_t * times)
{
    ...
    ret = fish_send_command (path_element->vclass, super, OPT_FLUSH, SUP->scr_utime,
...
fish_unlink (const vfs_path_t * vpath)
{
    ...
    ret =
        fish_send_command (path_element->vclass, super, OPT_FLUSH, SUP->scr_unlink,
                           "FISH_FILENAME=%s;\n", rpath);
...
fish_exists (const vfs_path_t * vpath)
{
    ...
    ret =
        fish_send_command (path_element->vclass, super, OPT_FLUSH, SUP->scr_exists,
                           "FISH_FILENAME=%s;\n", rpath);
...
fish_mkdir (const vfs_path_t * vpath, mode_t mode)
{
    ...
    ret =
        fish_send_command (path_element->vclass, super, OPT_FLUSH, SUP->scr_mkdir,
                           "FISH_FILENAME=%s;\n", rpath);
    g_free (rpath);
    ...
    {
        path_element->vclass->verrno = EACCES;
        return -1;
    }
    return 0;
}
...
fish_rmdir (const vfs_path_t * vpath)
{
    ...
    ret =
        fish_send_command (path_element->vclass, super, OPT_FLUSH, SUP->scr_rmdir,
                           "FISH_FILENAME=%s;\n", rpath);
...
init_fish (void)
{
    ...
    fish_subclass.flags = (vfs_subclass_flags_t)(VFS_S_REMOTE | VFS_S_USETMP);
