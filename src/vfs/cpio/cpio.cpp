    ...
    {                           /* !entry */
        ...
        else
        {
            if (inode != NULL)
            {
                ...
                inode->linkname = (char*)g_malloc (st->st_size + 1);
...
cpio_read_bin_head (struct vfs_class *me, struct vfs_s_super *super)
{
    ...
    name = (char*)g_malloc (u.buf.c_namesize);
...
cpio_read_oldc_head (struct vfs_class *me, struct vfs_s_super *super)
{
    ...
    name = (char*)g_malloc (hd.c_namesize);
                   ...
                   const vfs_path_element_t * vpath_element)
{
    ...
    if (cpio_open_cpio_file (vpath_element->vclass, super, vpath) == -1)
        return -1;
    ...
    while (TRUE)
    {
        ...
        status = cpio_read_head (vpath_element->vclass, super);
        if (status < 0)
            return (-1);
...
{
    struct stat *archive_stat = (struct stat*) cookie; /* stat of main archive */
