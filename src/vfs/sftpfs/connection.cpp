...
sftpfs_recognize_auth_types (struct vfs_s_super *super)
{
    ...
    if ((strstr (userauthlist, "password") != NULL
        ...
        super_data->auth_type = (sftpfs_auth_type_t)(super_data->auth_type | PASSWORD);

    if (strstr (userauthlist, "publickey") != NULL && (super_data->config_auth_type & PUBKEY) != 0)
        super_data->auth_type = (sftpfs_auth_type_t)(super_data->auth_type | PUBKEY);

    if ((super_data->config_auth_type & AGENT) != 0)
        super_data->auth_type = (sftpfs_auth_type_t)(super_data->auth_type | AGENT);
}
