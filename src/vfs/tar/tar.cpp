...
tar_read_header (struct vfs_class *me, struct vfs_s_super *archive, int tard, size_t * h_size)
{
  ...
  recurse:
    ...
    if (header->header.linkflag == LF_LONGNAME || header->header.linkflag == LF_LONGLINK)
    {
        ...
        g_free (*longp);
        bp = *longp = (char*)g_malloc (*h_size + 1);
                  ...
                  const vfs_path_element_t * vpath_element)
{
    ...
    /* Open for reading */
    tard = tar_open_archive_int (vpath_element->vclass, vpath, archive);
    if (tard == -1)
        return -1;
    ...
    while (TRUE)
    {
        ...
        status = tar_read_header (vpath_element->vclass, archive, tard, &h_size);
