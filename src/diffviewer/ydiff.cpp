...
f_dopen (int fd)
{
    ...
    fs = (FBUF *)g_try_malloc (sizeof (FBUF));
    if (fs == NULL)
        return NULL;

    fs->buf = (char*)g_try_malloc (FILE_READ_BUF);
    if (fs->buf == NULL)
    {
        g_free (fs);
        return NULL;
            ...
            for (i = 0; i < dview->a[DIFF_LEFT]->len; i++)
            {
                ...
                if (p->line && q->line && p->ch == CHG_CH)
                {
                    ...
                    if (h != NULL)
                    {
                        ...
                        runresult =
                            hdiff_scan ((char*)p->p, p->u.len, (char*)q->p, q->u.len, HDIFF_MINCTX, h,
                                        HDIFF_DEPTH);
                        if (!runresult)
                        {
                            g_array_free (h, TRUE);
...
#ifdef HAVE_CHARSET
                    if (dview->utf8)
                        k = dview_str_utf8_offset_to_pos ((char*)p->p, width);
                    else
#endif
                        k = width;

                    cvt_mgeta ((char*)p->p, p->u.len, buf, k, skip, tab_size, show_cr,
                               (GArray *)g_ptr_array_index (dview->hdiff, i), ord, att);
                    tty_gotoyx (r + j, c);
                    col = 0;
...
#endif
                    k = width;
                cvt_mget ((char*)p->p, p->u.len, buf, k, skip, tab_size, show_cr);
            }
            else
...
dview_status (const WDiff * dview, diff_place_t ord, int width, int c)
{
    ...
    path = vfs_path_to_str_flags (vpath, 0, (vfs_path_flag_t)(VPF_STRIP_HOME | VPF_STRIP_PASSWORD));
    vfs_path_free (vpath);
...
dview_update (WDiff * dview)
{
    ...
    if (width2 > 2)
    {
        dview_status (dview, (diff_place_t)(dview->ord ^ 1), width2, width1);
        dview_display_file (dview, (diff_place_t)(dview->ord ^ 1), 2, width1 + 1, height - 2, width2 - 2);
    }
}
...
dview_execute_cmd (WDiff * dview, long command)
{
    ...
    case CK_Tab8:
        ...
        break;
    case CK_Swap:
        dview->ord = (diff_place_t)(dview->ord ^ 1);
        break;
    case CK_Redo:
        dview_redo (dview);
        break;
    ...
    case CK_MergeOther:
        ...
        break;
    case CK_EditOther:
        dview_edit (dview, (diff_place_t)(dview->ord ^ 1));
        break;
    case CK_Search:
...
diff_view (const char *file1, const char *file2, const char *label1, const char *label2)
{
    ...
    w->options = (widget_options_t)(w->options | WOP_SELECTABLE);
