...
editcmd_dialog_raw_key_query (const char *heading, const char *query, gboolean cancel)
{
    ...
    raw_dlg =
        dlg_create (TRUE, 0, 0, cancel ? 7 : 5, w, (widget_pos_flags_t)(WPOS_CENTER | WPOS_TRYUP), FALSE, dialog_colors,
    ...
    if (cancel)
    {
        ...
        add_widget_autopos (raw_dlg, button_new (y, 1, B_CANCEL, NORMAL_BUTTON, _("Cancel"), NULL),
                            (widget_pos_flags_t)(WPOS_KEEP_TOP | WPOS_CENTER_HORZ), NULL);
    }
...
char *
editcmd_dialog_completion_show (const WEdit * edit, int max_len, GString ** completion, int num_compl)
{
    ...
    for (i = num_compl - 1; i >= 0; i--)        /* reverse order */
        listbox_add_item (compl_list, LISTBOX_APPEND_AT_END, 0, (char *) completion[i]->str, NULL,
                          FALSE);
