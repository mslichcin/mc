...
edit_save_mode_callback (Widget * w, Widget * sender, widget_msg_t msg, int parm, void *data)
{
    switch (msg)
    {
    case widget_msg_t::CHANGED_FOCUS:
...
{
    edit_search_line_t search_line_type = (edit_search_line_t)0;

    if (search->search_type != MC_SEARCH_T_REGEX)
        return search_line_type;

    if (*search->original == '^')
        search_line_type = (edit_search_line_t)(search_line_type | AT_START_LINE);

    if (search->original[search->original_len - 1] == '$')
        search_line_type = (edit_search_line_t)(search_line_type | AT_END_LINE);
    return search_line_type;
}
...
edit_get_block (WEdit * edit, off_t start, off_t finish, off_t * l)
{
    ...
    r = s = (unsigned char *)g_malloc0 (finish - start + 1);
    if (edit->column_highlight)
    {
        *l = 0;
...
edit_collect_completions (WEdit * edit, off_t word_start, gsize word_len,
                          char *match_expr, GString ** completion, gsize * num)
{
    gsize len = 0;
    gsize max_len = 0;
    gsize i;
    ...
    while (mc_search_run (srch, (void *) &esm, start + 1, last_byte, &len))
    {
        ...
        {
            if (strncmp
                ((char *) &completion[i]->str[word_len],
                 (char *) &temp->str[word_len], MAX (len, completion[i]->len) - word_len) == 0)
            {
                GString *that = completion[i];
                for (++i; i < *num; i++)
                    completion[i - 1] = completion[i];
                completion[*num - 1] = that;
                skip = 1;
        ...
        {
            g_string_free (completion[0], TRUE);
            for (i = 1; i < *num; i++)
                completion[i - 1] = completion[i];
            (*num)--;
        }
...
#endif
        completion[(*num)++] = g_string_new_len (temp->str, temp->len);
        start += len;
...
edit_get_macro (WEdit * edit, int hotkey, const macros_t ** macros, guint * indx)
{
    ...
    result = (macros_t *)bsearch (&search_macro, macros_list->data, macros_list->len,
...
edit_load_cmd (WDialog * h)
{
    ...
    exp = input_expand_dialog (_("Load"), _("Enter file name:"),
                               ...
                               (input_complete_t)(INPUT_COMPLETE_FILENAMES | INPUT_COMPLETE_CD));
...
edit_block_move_cmd (WEdit * edit)
{
    ...
    else
    {
        ...
        copy_buf = (unsigned char *)g_malloc0 (end_mark - start_mark);
        edit_cursor_move (edit, start_mark - edit->buffer.curs1);
        edit_scroll_screen_over_cursor (edit);
...
edit_save_block (WEdit * edit, const char *filename, off_t start, off_t finish)
{
    ...
    else
    {
        ...
        len = finish - start;
        buf = (unsigned char *)g_malloc0 (TEMP_BUF_LEN);
        while (start != finish)
        {
...
edit_ext_cmd (WEdit * edit)
{
    ...
    exp =
        input_dialog (_("Paste output of external command"),
                      ...
                      (input_complete_t)(INPUT_COMPLETE_FILENAMES | INPUT_COMPLETE_VARIABLES | INPUT_COMPLETE_USERNAMES
...
edit_complete_word_cmd (WEdit * edit)
{
    ...
    off_t word_start = 0;
    GString *match_expr;
    GString *completion[MAX_WORD_COMPLETIONS];       /* completions */
    ...
    max_len =
        edit_collect_completions (edit, word_start, word_len, match_expr->str, (GString **) & completion,
                                  &num_compl);
    ...
    if (num_compl > 0)
    {
        ...
        if (num_compl == 1)
            edit_complete_word_insert_recoded_completion (edit, completion[0]->str, word_len);
    ...
    for (i = 0; i < num_compl; i++)
        g_string_free (completion[i], TRUE);
}
