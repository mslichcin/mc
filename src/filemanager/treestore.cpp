...
tree_store_add_entry (const vfs_path_t * name)
{
    ...
    new_entry->name = vfs_path_clone (name);
    new_entry->sublevel = vfs_path_tokens_count (new_entry->name);
    {
        const char *new_name;

        new_name = vfs_path_get_last_path_str (new_entry->name);
        new_entry->subname = strrchr (new_name, PATH_SEP);
        if (new_entry->subname == NULL)
            new_entry->subname = new_name;
        else
            new_entry->subname++;
    }
    if (new_entry->next)
        submask = new_entry->next->submask;
    else
        submask = 0;
    submask |= 1 << new_entry->sublevel;
    submask &= (2 << new_entry->sublevel) - 1;
    new_entry->submask = submask;
    new_entry->mark = FALSE;

    /* Correct the submasks of the previous entries */
    current = new_entry->prev;
    while (current && current->sublevel > new_entry->sublevel)
    {
        current->submask |= 1 << new_entry->sublevel;
...
should_skip_directory (const vfs_path_t * vpath)
{
    ...
    for (l = special_dirs; l != NULL; l = g_list_next (l))
        if (strncmp (vfs_path_as_str (vpath), (char*)l->data, strlen ((char*)l->data)) == 0)
        {
            ret = TRUE;
            break;
        }
