    ...
    for (format = home; format != NULL && length != width; format = format->next)
    {
        if (format->string_fn != NULL)
        {
                ...
                if (i > -1)
                {
                    ...
                    if (str_len > len)
                    {
                        ...
                        if (str_length (txt + name_offset) > len)
                            res = (filename_scroll_flag_t)(res | FILENAME_SCROLL_RIGHT);
                    }
                }
            }
            ...
            if (!isstatus && panel->content_shift > -1)
                prepared_text =
                    str_fit_to_term (txt + name_offset, len, (align_crt_t)(HIDE_FIT (format->just_mode)));
            else
...
panel_correct_path_to_show (const WPanel * panel)
{
    ...
    if (elements_count > 1 && (strcmp (path_element->vclass->name, "cpiofs") == 0 ||
                               strcmp (path_element->vclass->name, "extfs") == 0 ||
                               strcmp (path_element->vclass->name, "tarfs") == 0))
    {
    ...
    return_path =
        vfs_path_to_str_flags (last_vpath, 0,
                               (vfs_path_flag_t)(VPF_STRIP_HOME | VPF_STRIP_PASSWORD | VPF_HIDE_CHARSET));
        ...
        for (i = 0; panel_fields[i].id != NULL; i++)
        {
            ...
            if (set_justify)
            {
                if (IS_FIT (darr->just_mode))
                    darr->just_mode = (align_crt_t)MAKE_FIT (justify);
                else
                             ...
                             gboolean do_select)
{
    ...
    /* result flags */
    panels_options.select_flags = (panel_select_flags_t)0;
    if (case_sens)
        panels_options.select_flags = (panel_select_flags_t)(panels_options.select_flags | SELECT_MATCH_CASE);
    if (files_only)
        panels_options.select_flags = (panel_select_flags_t)(panels_options.select_flags | SELECT_FILES_ONLY);
    if (shell_patterns)
        panels_options.select_flags = (panel_select_flags_t)(panels_options.select_flags | SELECT_SHELL_PATTERNS);
}
...
{
    panel->list_format = (list_format_t)((panel->list_format + 1) % LIST_FORMATS);
...
chdir_other_panel (WPanel * panel)
{
    ...
    else
    {
        ...
        sel_entry = (char*)strrchr (vfs_path_get_last_path_str (panel->cwd_vpath), PATH_SEP);
    }
    ...
    case MSG_MOUSE_DOWN:
        if (event->y == 0)
        {
            ...
            if (event->x == 1)
                ...
                send_message (midnight_dlg, NULL, widget_msg_t::ACTION, CK_ShowHidden, NULL);
            else
            {
...
update_one_panel (int which, panel_update_flags_t flags, const char *current_file)
{
    if (get_display_type (which) == view_listing)
    {
        ...
        if (panel->is_panelized)
            flags = (panel_update_flags_t)( flags & ~UP_RELOAD);
