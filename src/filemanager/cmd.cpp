...
compare_files (const vfs_path_t * vpath1, const vfs_path_t * vpath2, off_t size)
{
    ...
    if (file1 >= 0)
    {
        ...
        if (file2 >= 0)
        {
            ...
            /* Ugly if jungle */
            data1 = (char*)mmap (0, size, PROT_READ, MAP_FILE | MAP_PRIVATE, file1, 0);
            if (data1 != (char *) -1)
            {
                char *data2;

                data2 = (char*)mmap (0, size, PROT_READ, MAP_FILE | MAP_PRIVATE, file2, 0);
                if (data2 != (char *) -1)
                {
                    rotate_dash (TRUE);
         ...
         const char *history_name, const char *prefix, int to_home, gboolean strip_password)
{
    ...
    machine =
        input_dialog_help (text, xtext, help, history_name, INPUT_LAST_TEXT, strip_password,
                           (input_complete_t)(INPUT_COMPLETE_FILENAMES | INPUT_COMPLETE_CD | INPUT_COMPLETE_HOSTNAMES |
                         ...
                         char **user, char **status)
{
    ...
    p->list_format = (list_format_t)list_format;
...
view_filtered_cmd (void)
{
    ...
    command =
        input_dialog (_("Filtered view"),
                      ...
                      (input_complete_t)(INPUT_COMPLETE_FILENAMES | INPUT_COMPLETE_COMMANDS));
...
reread_cmd (void)
{
    ...
    update_panels ((panel_update_flags_t)(UP_RELOAD | flag), UP_KEEPSEL);
    repaint_screen ();
}
...
compare_dirs_cmd (void)
{
    ...
    thorough_flag = (CompareMode)choice;
...
single_dirsize_cmd (void)
{
    ...
    if (panels_options.mark_moves_down)
        send_message (panel, NULL, widget_msg_t::ACTION, CK_Down, NULL);
