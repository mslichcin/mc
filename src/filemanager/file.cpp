...
check_hardlinks (const vfs_path_t * src_vpath, const vfs_path_t * dst_vpath, struct stat *pstat)
{
    ...
    my_vfs = vfs_path_get_by_index (src_vpath, -1)->vclass;
...
/* *INDENT-ON* */
    ...
    if (mc_global.we_are_background)
        return (FileProgressStatus) parent_call (pntr.p, NULL, 3, strlen (fmt), fmt, strlen (a), a, strlen (b), b);
#endif
...
real_query_recursive (file_op_context_t * ctx, enum OperationMode mode, const char *s)
{
    if (ctx->recursive_result < RECURSIVE_ALWAYS)
    {
        ...
        ctx->recursive_result = (FileCopyMode)
...
/* *INDENT-ON* */
    ...
    if (mc_global.we_are_background)
        return (FileProgressStatus) parent_call (pntr.p, NULL, 1, strlen (str), str);
    else
...
/* *INDENT-ON* */
    ...
    if (mc_global.we_are_background)
        return (FileProgressStatus) parent_call (pntr.p, ctx, 1, strlen (s), s);
    else
...
/* *INDENT-ON* */
    ...
    if (mc_global.we_are_background)
        return (FileProgressStatus) parent_call (pntr.p, ctx, 3, strlen (destname), destname,
                 ...
                 const char *s, const char *d)
{
  ...
  ret:
    erase_list = (GSList *) free_linklist (erase_list);
  ret_fast:
    vfs_path_free (src_vpath);
    vfs_path_free (dst_vpath);
    return return_status;
        ...
        if (!ok)
        {
            ...
            if (!panel->is_panelized)
            {
                ...
                if (get_other_type () == view_listing && other_panel->is_panelized)
                    flags = (panel_update_flags_t)(flags | UP_ONLY_CURRENT);
        ...
        if (temp != NULL)
        {
            ...
            case OP_COPY:
                ...
                if (is_file)
                    ...
                    value = copy_dir_dir (tctx, ctx, src, dest, TRUE, FALSE, FALSE, NULL);
                dest_dirs = (GSList*)free_linklist (dest_dirs);
                break;
              ...
              gboolean toplevel, gboolean move_over, gboolean do_delete, GSList * parent_dirs)
{
  ...
  retry_src_stat:
    ...
    lp->vfs = vfs_path_get_by_index (src_vpath, -1)->vclass;
    lp->ino = cbuf.st_ino;
    lp->dev = cbuf.st_dev;
  ...
  retry_dst_stat:
    ...
    if (do_mkdir)
    {
        ...
        lp->vfs = vfs_path_get_by_index (dst_vpath, -1)->vclass;
        lp->ino = buf.st_ino;
        lp->dev = buf.st_dev;
...
panel_operate (void *source_panel, FileOperation operation, gboolean force_single)
{
    ...
    linklist = (GSList*)free_linklist (linklist);
    dest_dirs = (GSList*)free_linklist (dest_dirs);
