                 ...
                 char **ignore_dirs, char **pattern, char **content)
{
    ...
    in_start =
        input_new (y1, x1, input_colors, cols - b0 - 7, in_start_dir, "start",
                   (input_complete_t)(INPUT_COMPLETE_CD | INPUT_COMPLETE_FILENAMES));
...
insert_file (const char *dir, const char *file, gsize start, gsize end)
{
    ...
    location = (find_match_location_t *)g_malloc (sizeof (*location));
...

    {
        ...
        while (!ret_val)
        {
            ...
            while (TRUE)
            {
                ...
                {
                    strbuf_size += 128;
                    strbuf = (char*)g_realloc (strbuf, strbuf_size);
                }
         ...
         const char *pattern, const char *content, char **dirname, char **filename)
{
    ...
    if (return_value == B_PANELIZE && *filename)
    {
             ...
             i++, entry = g_list_next (entry))
        {
            ...
            find_match_location_t *location = (find_match_location_t *)le->data;
            char *p;
