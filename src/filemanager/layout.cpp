...
#define XTRACT(i) (*check_options[i].variable != 0), check_options[i].text
    ...
    /* "Console output" groupbox */
    {
        ...
        disabled = mc_global.tty.console_flag != '\0' ? WST_DEFAULT : WST_DISABLED;

        w = WIDGET (groupbox_new (8, 3, 3, l1, title2));
        w->state = (widget_state_t)(w->state | disabled);
...
#undef panelswap
    ...
    else
    {
        ...
        panels[1].type = panel_view_mode_t(tmp_type);
...
title_path_prepare (char **path, char **login)
{
    ...
    *path =
        vfs_path_to_str_flags (current_panel->cwd_vpath, 0, (vfs_path_flag_t)(VPF_STRIP_HOME | VPF_STRIP_PASSWORD));
