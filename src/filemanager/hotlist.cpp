...
} hotlist_but[] =
{
    ...
    { B_ENTER, DEFPUSH_BUTTON, 0, 0, 0, N_("Change &to"),
            (hotlist_t)(LIST_HOTLIST | LIST_VFSLIST | LIST_MOVELIST), (widget_pos_flags_t)(WPOS_KEEP_LEFT | WPOS_KEEP_BOTTOM) },
#ifdef ENABLE_VFS
    { B_FREE_ALL_VFS, NORMAL_BUTTON, 0, 20, 0, N_("&Free VFSs now"),
            LIST_VFSLIST, (widget_pos_flags_t)(WPOS_KEEP_LEFT | WPOS_KEEP_BOTTOM) },
    { B_REFRESH_VFS, NORMAL_BUTTON, 0, 43, 0, N_("&Refresh"),
            LIST_VFSLIST, (widget_pos_flags_t)(WPOS_KEEP_LEFT | WPOS_KEEP_BOTTOM) },
#endif
    { B_ADD_CURRENT, NORMAL_BUTTON, 0, 20, 0, N_("&Add current"),
            LIST_HOTLIST, (widget_pos_flags_t)(WPOS_KEEP_LEFT | WPOS_KEEP_BOTTOM) },
    { B_UP_GROUP, NORMAL_BUTTON, 0, 42, 0, N_("&Up"),
            (hotlist_t)(LIST_HOTLIST | LIST_MOVELIST), (widget_pos_flags_t)(WPOS_KEEP_LEFT | WPOS_KEEP_BOTTOM) },
    { B_CANCEL, NORMAL_BUTTON, 0, 53, 0, N_("&Cancel"),
            (hotlist_t)(LIST_HOTLIST | LIST_VFSLIST | LIST_MOVELIST), (widget_pos_flags_t)(WPOS_KEEP_RIGHT | WPOS_KEEP_BOTTOM) },
    ...
    { B_INSERT, NORMAL_BUTTON, 1, 0, 0, N_("&Insert"),
            LIST_MOVELIST, (widget_pos_flags_t)(WPOS_KEEP_LEFT | WPOS_KEEP_BOTTOM) },
...
init_hotlist (hotlist_t list_type)
{
    ...
    add_widget_autopos (hotlist_dlg, path_box, (widget_pos_flags_t)(WPOS_KEEP_BOTTOM | WPOS_KEEP_HORZ), NULL);

    pname = label_new (y + 1, UX + 2, "");
    add_widget_autopos (hotlist_dlg, pname, (widget_pos_flags_t)(WPOS_KEEP_BOTTOM | WPOS_KEEP_LEFT), NULL);
        ...
        if ((hotlist_but[i].type & list_type) != 0)
            add_widget_autopos (hotlist_dlg,
                                button_new (y + hotlist_but[i].y, UX + hotlist_but[i].x,
                                            hotlist_but[i].ret_cmd, (button_flags_t)hotlist_but[i].flags,
...
add2hotlist (char *label, char *directory, enum HotListType type, listbox_append_t pos)
{
    ...
    output = g_new0 (struct hotlist, 1);

    output->type = type;
    output->label = label;
    output->directory = directory;
    output->up = current_group;

    if (type == HL_TYPE_GROUP)
    {
        current_group = output;
        add_dotdot_to_list ();
        current_group = output->up;
    }

    if (current_group->head == NULL)
    {
        /* first element in group */
        current_group->head = output;
    }
    ...
    if (hotlist_state.running && type != HL_TYPE_COMMENT && type != HL_TYPE_DOTDOT)
    {
        if (type == HL_TYPE_GROUP)
        {
            ...
            lbl = g_strconcat ("->", output->label, (char *) NULL);
            listbox_add_item (l_hotlist, pos, 0, lbl, output, FALSE);
            g_free (lbl);
        }
        else
            listbox_add_item (l_hotlist, pos, 0, output->label, output, FALSE);
                     ...
                     const char *help, char **r1, char **r2)
{
    quick_widget_t quick_widgets[] = {
        ...
        QUICK_LABELED_INPUT (text2, input_label_above, *r2, "input-lbl", r2, NULL,
                             FALSE, FALSE, (input_complete_t)(INPUT_COMPLETE_FILENAMES | INPUT_COMPLETE_CD)),
