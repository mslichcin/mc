...
register_task_running (file_op_context_t * ctx, pid_t pid, int fd, int to_child, char *info)
{
    ...
    newTask = g_new (TaskList, 1);
    newTask->pid = pid;
    newTask->info = info;
    newTask->state = Task_Running;
    newTask->next = task_list;
    newTask->fd = fd;
    newTask->to_child_fd = to_child;
    task_list = newTask;
