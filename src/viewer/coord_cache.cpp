...
mcview_ccache_add_entry (coord_cache_t * cache, size_t pos, const coord_cache_entry_t * entry)
{
    ...
    if (cache->size == cache->capacity)
    {
        ...
        cache->cache = (coord_cache_entry_t **)g_realloc (cache->cache, cache->capacity * sizeof (*cache->cache));
    }
    ...
    cache->cache[pos] = (coord_cache_entry_t *)g_memdup (entry, sizeof (*entry));
    cache->size++;
}
...
coord_cache_new (void)
{
    ...
    cache->size = 0;
    cache->capacity = CACHE_CAPACITY_DELTA;
    cache->cache = (coord_cache_entry_t **)g_malloc0 (cache->capacity * sizeof (*cache->cache));
