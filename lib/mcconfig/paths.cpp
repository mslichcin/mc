...
mc_config_mkdir (const char *directory_name, GError ** mcerror)
{
    ...
    if ((!g_file_test (directory_name, (GFileTest)(G_FILE_TEST_EXISTS | G_FILE_TEST_IS_DIR))) &&
...
mc_config_deprecated_dir_present (void)
{
    ...
    is_present = g_file_test (old_dir, (GFileTest)(G_FILE_TEST_EXISTS | G_FILE_TEST_IS_DIR));
    g_free (old_dir);
