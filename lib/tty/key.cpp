...
xmouse_get_event (Gpm_Event * ev, gboolean extended)
{
    ...
    else
    {
        ...
        char c;
        btn = ev->x = ev->y = 0;
        ev->type = (Gpm_Etype) 0;           /* In case we return on an invalid sequence */
    ...
    if (btn == 3)
    {
        if (last_btn != 0)
        {
            ...
            {
                ev->type = (Gpm_Etype)(GPM_UP | (GPM_SINGLE << clicks));
                GET_TIME (tv1);
            }
            ev->buttons = 0;
            last_btn = 0;
...
lookup_keyname (const char *name, int *idx)
{
    if (name[0] != '\0')
    {
        ...
        res = (const key_code_name_t **) bsearch (&keyp, key_conv_tab_sorted, key_conv_tab_size,
...
add_select_channel (int fd, select_fn callback, void *info)
{
    ...
    newSL = g_new (SelectList, 1);
    newSL->fd = fd;
    newSL->callback = callback;
    newSL->info = info;
    newSL->next = select_list;
    select_list = newSL;
}
...
{
    int c;
    static key_def *this_key = NULL, *parent;
  ...
  nodelay_try_again:
    ...
    if (no_delay)
    {
        ...
        if (c == -1)
        {
            if (this_key != NULL && parent != NULL && parent->action == MCKEY_ESCAPE && old_esc_mode)
            {
    ...
    while (this_key != NULL)
    {
        ...
        /* c != this_key->ch. Try other keys with this_key prefix */
        if (this_key->next != NULL)
        {
            this_key = this_key->next;
            continue;
        }
