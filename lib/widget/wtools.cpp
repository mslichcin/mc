...
query_dialog (const char *header, const char *text, int flags, int count, ...)
{
    ...
    widget_pos_flags_t pos_flags =
        (flags & D_CENTER) != 0 ? (widget_pos_flags_t)(WPOS_CENTER | WPOS_TRYUP) : WPOS_KEEP_DEFAULT;
    ...
    if (count > 0)
    {
        ...
        add_widget_autopos (query_dlg, label_new (2, 3, text), (widget_pos_flags_t)(WPOS_KEEP_TOP | WPOS_CENTER_HORZ),
                            NULL);
...
simple_status_msg_init_cb (status_msg_t * sm)
{
    ...
    add_widget_autopos (sm->dlg, ssm->label, (widget_pos_flags_t)(WPOS_KEEP_TOP | WPOS_CENTER_HORZ), NULL);
    add_widget (sm->dlg, hline_new (y++, -1, -1));
    b = WIDGET (button_new (y++, 3, B_CANCEL, NORMAL_BUTTON, b_name, NULL));
    add_widget_autopos (sm->dlg, b, (widget_pos_flags_t)(WPOS_KEEP_TOP | WPOS_CENTER_HORZ), NULL);
