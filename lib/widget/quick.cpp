...
quick_dialog_skip (quick_dialog_t * quick_dlg, int nskip)
{
    ...
    if (quick_dlg->x == -1 || quick_dlg->y == -1)
        dd = dlg_create (TRUE, 0, 0, y + 3, quick_dlg->cols, (widget_pos_flags_t)(WPOS_CENTER | WPOS_TRYUP), FALSE,
    ...
    for (i = 0; i < widgets->len; i++)
    {
        ...
        if (item->widget != NULL)
        {
            ...
            item->widget->options = (widget_options_t)(item->widget->options | item->quick_widget->options);       /* FIXME: cannot reset flags, setup only */
            item->widget->state = (widget_state_t)(item->widget->state | item->quick_widget->state);   /* FIXME: cannot reset flags, setup only */
