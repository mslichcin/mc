...
{
    if (enable)
        w->options = (widget_options_t)(w->options | options);
    else
        w->options = (widget_options_t)(w->options & ~options);
}
...
widget_set_state (Widget * w, widget_state_t state, gboolean enable)
{
    ...
    if (enable)
        w->state = (widget_state_t)(w->state | state);
    else
        w->state = (widget_state_t)(w->state & ~state);
    ...
    if (enable)
    {
        ...
        if ((state & WST_CONSTRUCT) != 0)
            w->state = (widget_state_t)(w->state & ~(WST_ACTIVE | WST_SUSPENDED | WST_CLOSED));
        else if ((state & WST_ACTIVE) != 0)
            w->state = (widget_state_t)(w->state & ~(WST_CONSTRUCT | WST_SUSPENDED | WST_CLOSED));
        else if ((state & WST_SUSPENDED) != 0)
            w->state = (widget_state_t)(w->state & ~(WST_CONSTRUCT | WST_ACTIVE | WST_CLOSED));
        else if ((state & WST_CLOSED) != 0)
            w->state = (widget_state_t)(w->state & ~(WST_CONSTRUCT | WST_ACTIVE | WST_SUSPENDED));
    }
