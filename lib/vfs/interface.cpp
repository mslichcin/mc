...
mc_open (const vfs_path_t * vpath, int flags, ...)
{
    ...
    if (vfs_path_element_valid (path_element) && path_element->vclass->open != NULL)
    {
        void *info;
        /* open must be supported */
        info = path_element->vclass->open (vpath, flags, mode);
        if (info == NULL)
            errno = vfs_ferrno (path_element->vclass);
        else
            result = vfs_new_handle (path_element->vclass, info);
    }
    else
        errno = -EOPNOTSUPP;
...
\
    result = path_element->vclass->name != NULL ? path_element->vclass->name callarg : -1; \
    if (result == -1) \
        errno = path_element->vclass->name != NULL ? vfs_ferrno (path_element->vclass) : E_NOTSUPP; \
    return result; \
}
...
mc_symlink (const vfs_path_t * vpath1, const vfs_path_t * vpath2)
{
    ...
    if (vpath1 != NULL)
    {
        ...
        {
            result =
                path_element->vclass->symlink != NULL ?
                path_element->vclass->symlink (vpath1, vpath2) : -1;

            if (result == -1)
                errno =
                    path_element->vclass->symlink != NULL ?
                    vfs_ferrno (path_element->vclass) : E_NOTSUPP;
        }
    }
    return result;
}
...
ssize_t mc_##name (int handle, C char *buf, size_t count) \
{ \
...
\
    if (!vfs_path_element_valid (path_element1) || !vfs_path_element_valid (path_element2) || \
        path_element1->vclass != path_element2->vclass) \
    { \
        errno = EXDEV; \
        return -1; \
    }\
\
    result = path_element1->vclass->name != NULL \
        ? path_element1->vclass->name (vpath1, vpath2) \
        : -1; \
    if (result == -1) \
        errno = path_element1->vclass->name != NULL ? vfs_ferrno (path_element1->vclass) : E_NOTSUPP; \
    return result; \
}
...
mc_setctl (const vfs_path_t * vpath, int ctlop, void *arg)
{
    ...
    if (vfs_path_element_valid (path_element))
        result =
            path_element->vclass->setctl != NULL ? path_element->vclass->setctl (vpath,
                                                                               ctlop, arg) : 0;
...
mc_opendir (const vfs_path_t * vpath)
{
    ...
    info = path_element->vclass->opendir ? (*path_element->vclass->opendir) (vpath) : NULL;

    if (info == NULL)
    {
        errno = path_element->vclass->opendir ? vfs_ferrno (path_element->vclass) : E_NOTSUPP;
        return NULL;
    }

    path_element->dir.info = (DIR*)info;
    ...
    handle = vfs_new_handle (path_element->vclass, vfs_path_element_clone (path_element));
...
mc_readdir (DIR * dirp)
{
    ...
    if (vfs->readdir)
    {
        entry = (dirent *)((*vfs->readdir) (vfs_path_element->dir.info));
        if (entry == NULL)
            return NULL;
...
mc_stat (const vfs_path_t * vpath, struct stat *buf)
{
    ...
    if (vfs_path_element_valid (path_element))
    {
        result = path_element->vclass->stat ? (*path_element->vclass->stat) (vpath, buf) : -1;
        if (result == -1)
            errno = path_element->vclass->name ? vfs_ferrno (path_element->vclass) : E_NOTSUPP;
    }
...
mc_lstat (const vfs_path_t * vpath, struct stat *buf)
{
    ...
    if (vfs_path_element_valid (path_element))
    {
        result = path_element->vclass->lstat ? (*path_element->vclass->lstat) (vpath, buf) : -1;
        if (result == -1)
            errno = path_element->vclass->name ? vfs_ferrno (path_element->vclass) : E_NOTSUPP;
    }
...
mc_getlocalcopy (const vfs_path_t * pathname_vpath)
{
    ...
    if (vfs_path_element_valid (path_element))
    {
        result = path_element->vclass->getlocalcopy != NULL ?
            path_element->vclass->getlocalcopy (pathname_vpath) :
                   ...
                   gboolean has_changed)
{
    ...
    if (vfs_path_element_valid (path_element))
        return_value = path_element->vclass->ungetlocalcopy != NULL ?
            path_element->vclass->ungetlocalcopy (pathname_vpath, local_vpath, has_changed) :
...
mc_chdir (const vfs_path_t * vpath)
{
    ...
    if (!vfs_path_element_valid (path_element) || path_element->vclass->chdir == NULL)
    {
        goto error_end;
    }

    result = (*path_element->vclass->chdir) (cd_vpath);
    ...
    current_vfs = path_element->vclass;
