                       ...
                       const char *a_path, int follow, int flags)
{
    ...
    custom_canonicalize_pathname (path, (CANON_PATH_FLAGS)(CANON_PATH_ALL & (~CANON_PATH_REMDOUBLEDOTS)));
...
vfs_s_inode_from_path (const vfs_path_t * vpath, int flags)
{
    ...
    ino =
        vfs_s_find_inode (path_element->vclass, super, q,
...
vfs_s_opendir (const vfs_path_t * vpath)
{
    ...
    {
        path_element->vclass->verrno = ENOTDIR;
        return NULL;
    }
    ...
    {
        path_element->vclass->verrno = EAGAIN;
        return NULL;
    }
#endif
...
vfs_s_readlink (const vfs_path_t * vpath, char *buf, size_t size)
{
    ...
    {
        path_element->vclass->verrno = EINVAL;
        return (-1);
    }

    if (ino->linkname == NULL)
    {
        path_element->vclass->verrno = EFAULT;
        return (-1);
    }
...
vfs_s_close (void *fh)
{
    ...
    if (FH->linear == LS_LINEAR_OPEN)
        MEDATA->linear_close (me, (vfs_file_handler_t *)fh);
    if (MEDATA->fh_close != NULL)
        res = MEDATA->fh_close (me, (vfs_file_handler_t *)fh);
    if ((MEDATA->flags & VFS_S_USETMP) != 0 && FH->changed && MEDATA->file_store != NULL)
    {
        ...
        {
            res = MEDATA->file_store (me, (vfs_file_handler_t *)fh, s, FH->ino->localname);
            g_free (s);
        }
    ...
    if (MEDATA->fh_free_data != NULL)
        MEDATA->fh_free_data ((vfs_file_handler_t *)fh);
    g_free (fh);
    return res;
}
...
vfs_s_getlocalcopy (const vfs_path_t * vpath)
{
    ...
    fh = (vfs_file_handler_t *)vfs_s_open (vpath, O_RDONLY, 0);

    if (fh != NULL)
    {
        const struct vfs_class *me;

        me = vfs_path_get_by_index (vpath, -1)->vclass;
    ...
    case VFS_SETCTL_STALE_DATA:
        {
            ...
            else
            {
                ...
                vfs_s_invalidate (path_element->vclass, ino->super);
            }
            return 1;
        }
    case VFS_SETCTL_LOGFILE:
        ((struct vfs_s_subclass *) path_element->vclass->data)->logfile = fopen ((char *) arg, "w");
        return 1;
    case VFS_SETCTL_FLUSH:
        ((struct vfs_s_subclass *) path_element->vclass->data)->flush = 1;
        return 1;
    default:
        return 0;
    }
...
vfs_get_super_by_vpath (const vfs_path_t * vpath)
{
    ...
    path_element = vfs_path_get_by_index (vpath, -1);
    subclass = (struct vfs_s_subclass *) path_element->vclass->data;
    if (subclass == NULL)
        return NULL;
...
vfs_s_get_path (const vfs_path_t * vpath, struct vfs_s_super **archive, int flags)
{
    ...
    {
        path_element->vclass->verrno = EIO;
        return NULL;
    }

    super = vfs_s_new_super (path_element->vclass);
    ...
    if (result == -1)
    {
        vfs_s_free_super (path_element->vclass, super);
        path_element->vclass->verrno = EIO;
        return NULL;
    }
    if (super->name == NULL)
    ...
    vfs_s_insert_super (path_element->vclass, super);
    vfs_stamp_create (path_element->vclass, super);
...
vfs_s_open (const vfs_path_t * vpath, int flags, mode_t mode)
{
    ...
    if (q == NULL)
        return NULL;
    ino = vfs_s_find_inode (path_element->vclass, super, q, LINK_FOLLOW, FL_NONE);
    if (ino != NULL && (flags & (O_CREAT | O_EXCL)) == (O_CREAT | O_EXCL))
    {
        path_element->vclass->verrno = EEXIST;
        return NULL;
    }
    ...
    if (ino == NULL)
    {
        ...
        if ((flags & O_CREAT) == 0 || path_element->vclass->write == NULL)
            return NULL;

        dirname = g_path_get_dirname (q);
        name = g_path_get_basename (q);
        dir = vfs_s_find_inode (path_element->vclass, super, dirname, LINK_FOLLOW, FL_DIR);
        if (dir == NULL)
        {
            g_free (dirname);
            g_free (name);
        ...
        ent = vfs_s_generate_entry (path_element->vclass, name, dir, 0755);
        ino = ent->ino;
        vfs_s_insert_entry (path_element->vclass, dir, ent);
        if ((VFSDATA (path_element)->flags & VFS_S_USETMP) != 0)
        {
            int tmp_handle;
            vfs_path_t *tmp_vpath;

            tmp_handle = vfs_mkstemps (&tmp_vpath, path_element->vclass->name, name);
    ...
    {
        path_element->vclass->verrno = EISDIR;
        return NULL;
    }
    ...
    else
    {
        ...
        if (s->fh_open != NULL && s->fh_open (path_element->vclass, fh, flags, mode) != 0)
        {
    ...
    if ((VFSDATA (path_element)->flags & VFS_S_USETMP) != 0 && fh->ino->localname != NULL)
    {
        ...
        {
            g_free (fh);
            path_element->vclass->verrno = errno;
            return NULL;
        }
    }

    /* i.e. we had no open files and now we have one */
    vfs_rmstamp (path_element->vclass, (vfsid) super);
    super->fd_usage++;
    fh->ino->st.st_nlink++;
    return fh;
}
...
vfs_getid (const vfs_path_t * vpath)
{
    ...
    path_element = vfs_path_get_by_index (vpath, -1);
    if (!vfs_path_element_valid (path_element) || path_element->vclass->getid == NULL)
        return NULL;

    return (*path_element->vclass->getid) (vpath);
}
