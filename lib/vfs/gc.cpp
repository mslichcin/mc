...
vfs_stamp_path (const char *path)
{
    ...
    id = vfs_getid (vpath);
    vfs_addstamp (path_element->vclass, id);
    vfs_path_free (vpath);
}
...
vfs_stamp_create (struct vfs_class *vclass, vfsid id)
{
    ...
    vfs_rmstamp (path_element->vclass, nvfsid);

    if (!(id == NULL || (path_element->vclass == vclass && nvfsid == id)))
    {
...
vfs_release_path (const vfs_path_t * vpath)
{
    ...
    path_element = vfs_path_get_by_index (vpath, -1);
    vfs_stamp_create (path_element->vclass, vfs_getid (vpath));
}
