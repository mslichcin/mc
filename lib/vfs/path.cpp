...
vfs_path_from_str_deprecated_parser (char *path)
{
    ...
    while ((vclass = _vfs_split_with_semi_skip_count (path, &local, &op, 0)) != NULL)
    {
        char *url_params;
        element = g_new0 (vfs_path_element_t, 1);
        element->vclass = vclass;
        if (local == NULL)
            local = "";
        ...
        url_params = (char*)strchr (op, ':');  /* skip VFS prefix */
        if (url_params != NULL)
        {
            *url_params = '\0';
            url_params++;
    ...
    {
        element = g_new0 (vfs_path_element_t, 1);
        element->vclass = (vfs_class *)g_ptr_array_index (vfs__classes_list, 0);
...
vfs_path_from_str_uri_parser (char *path)
{
    ...
    while ((url_delimiter = g_strrstr (path, VFS_PATH_URL_DELIMITER)) != NULL)
    {
        ...
        *url_delimiter = '\0';
        ...
        element = g_new0 (vfs_path_element_t, 1);
        element->vclass = vfs_prefix_to_class (vfs_prefix_start);
...
{
    if (((element->vclass->flags & VFSF_LOCAL) == 0 || ret_tokens->len > 0)
...
{ \
    if ((flags & VPF_STRIP_HOME) && element_index == 0 && (element->vclass->flags & VFSF_LOCAL) != 0) \
    { \
...
vfs_path_element_clone (const vfs_path_element_t * element)
{
    ...
    new_element->vclass = element->vclass;
...
vfs_path_serialize (const vfs_path_t * vpath, GError ** mcerror)
{
    ...
    for (element_index = 0; element_index < vfs_path_elements_count (vpath); element_index++)
    {
        ...
        mc_config_set_string_raw (cpath, groupname, "class-name", element->vclass->name);
#ifdef HAVE_CHARSET
...
vfs_path_element_build_pretty_path_str (const vfs_path_element_t * element)
{
    ...
    pretty_path = g_string_new (element->vclass->prefix);
