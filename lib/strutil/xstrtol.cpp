...
bkm_scale_by_power (uintmax_t * x, int base, int power)
{
    ...
    while (power-- != 0)
        err = (strtol_error_t) (err | bkm_scale (x, base));
    return err;
}
...
xstrtoumax (const char *s, char **ptr, int base, uintmax_t * val, const char *valid_suffixes)
{
    ...
    if (**p != '\0')
    {
        ...
        {
            *val = tmp;
            return (strtol_error_t)(err | LONGINT_INVALID_SUFFIX_CHAR);
        }
        ...
        err = (strtol_error_t)(err | overflow);
        *p += suffixes;
        if (**p != '\0')
            (strtol_error_t)(err | LONGINT_INVALID_SUFFIX_CHAR);
    }
