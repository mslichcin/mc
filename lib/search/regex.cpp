                                    ...
                                    GMatchInfo ** match_info, GError ** error)
{
    ...
    p = string_safe = (char*) g_malloc (string_len);
    ...
    *skip_len = 0;
    ...
    if (curr_str[0] == '\\' && replace_str->len > current_pos + 1)
    {
        ...
        *skip_len += 2;
        ...
        {
        case 'U':
            *replace_flags = (replace_transform_type_t) ( *replace_flags | REPLACE_T_UPP_TRANSFORM);
            *replace_flags = (replace_transform_type_t) ( *replace_flags & ~REPLACE_T_LOW_TRANSFORM);
            break;
        case 'u':
            *replace_flags = (replace_transform_type_t) ( *replace_flags | REPLACE_T_UPP_TRANSFORM_CHAR);
            break;
        case 'L':
            *replace_flags = (replace_transform_type_t) ( *replace_flags | REPLACE_T_LOW_TRANSFORM);
            *replace_flags = (replace_transform_type_t) ( *replace_flags & ~REPLACE_T_UPP_TRANSFORM);
            break;
        case 'l':
            *replace_flags = (replace_transform_type_t) ( *replace_flags | REPLACE_T_LOW_TRANSFORM_CHAR);
            break;
        case 'E':
                                       ...
                                       mc_search_cond_t * mc_search_cond)
{
...

    {
#ifdef SEARCH_TYPE_GLIB
        GError *mcerror = NULL;
        GRegexCompileFlags g_regex_options = (GRegexCompileFlags) (G_REGEX_OPTIMIZE | G_REGEX_DOTALL);

        if (str_isutf8 (charset) && mc_global.utf8_display)
        {
            if (!lc_mc_search->is_case_sensitive)
                g_regex_options = (GRegexCompileFlags)(g_regex_options | G_REGEX_CASELESS);
        }
        else
        {
            g_regex_options = (GRegexCompileFlags)(g_regex_options | G_REGEX_RAW);
        ...
        mc_search_cond->regex_handle =
            g_regex_new (mc_search_cond->str->str, g_regex_options, (GRegexMatchFlags) 0, &mcerror);
